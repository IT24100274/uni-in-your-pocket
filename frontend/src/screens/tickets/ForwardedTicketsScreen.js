...
import { getForwardedTickets } from "../../services/api";

const FILTERS = ["all", "forwarded", "in_progress", "resolved", "closed"];
...
const ForwardedTicketsScreen = ({ navigation }) => {
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      const res = await getForwardedTickets();
      setTickets(res.data);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Text style={styles.countText}>{filtered.length} ticket{filtered.length !== 1 ? "s" : ""} forwarded to you</Text>
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            <Text style={styles.dateText}>
              Forwarded: {new Date(item.forwardedAt).toLocaleDateString()}
            </Text>
          </TouchableOpacity>
        )}
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>
              {searchQuery.trim() ? "No tickets match your search" : "No tickets forwarded to you yet"}
            </Text>
          </View>
        }
      />
...
const styles = StyleSheet.create({
  ...
  cardMeta: { flexDirection: "row", gap: 6, marginBottom: 6 },
...
export default ForwardedTicketsScreen;